...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Outfit', sans-serif; -webkit-tap-highlight-color: transparent; outline: none; user-select: none; -webkit-user-select: none; }
        ...
        .target-lock-banner { background: rgba(0, 229, 255, 0.1); border: 1px dashed var(--neon-cyan); padding: 10px 12px; border-radius: 8px; margin-bottom: 20px; display: flex; align-items: center; gap: 12px; flex-shrink: 0; width: 100%; max-width: 600px; box-sizing: border-box; }
...
<body onclick="closeAllMenus()"> 
    ...
    <script>
            ...
            db.ref(`trackings/${currentAdminTargetPhone}/client_messages`).orderByChild('timestamp').limitToLast(100).on('value', (snap) => {
                ...
                if(snap.exists()) {
                    ...
                    snap.forEach(child => {
                        ...
                        // FIXED STRING REPLACEMENT CODE CRITICAL BUG
        ...
        // RESTORED: MISSING REPLY LOGIC TO PREVENT RUNTIME EXCEPTIONS
        ...
        function startClientWatchListeners() {
            ...
            db.ref(`trackings/${currentClientPhone}/client_messages`).orderByChild('timestamp').limitToLast(100).on('value', (snap) => {
                ...
                if(snap.exists()) {
                    ...
                    area.innerHTML = '<div style="font-size:12px; color:rgba(255,255,255,0.5); text-align:center; margin: auto; background:rgba(0,0,0,0.4); padding:5px 10px; border-radius:10px;">End-to-end encrypted chat started</div>';
                    isChatInitialLoad = false;
                }
            });
        }
